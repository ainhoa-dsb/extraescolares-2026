  ...
  categorias: [
    ...
    },
    {
      ...
      actividades: [
        ...
        actividad('Inteligencia emocional', 'Viernes 17:00-18:30', 'Actividad para reconocer emociones, mejorar la autoestima, practicar habilidades sociales y aprender estrategias de calma y convivencia.', '21 euros socios AMPA', '24 euros no socios AMPA'),
        actividad('Karate', 'Martes y jueves 15:30-16:30', 'Practica de karate adaptada a la edad para trabajar disciplina, equilibrio, coordinacion, respeto y control corporal.', '21 euros socios AMPA', '24 euros no socios AMPA'),
...
function crearFormularioExtraescolares() {
  ...
  CONFIG.categorias.forEach((categoria) => {
    ...
    form.addCheckboxItem()
      ...
      .setChoiceValues(categoria.actividades.map((item) => formatearOpcionFormulario(item, categoria.nombre)))
      .setRequired(false);
  });

  const opciones = obtenerActividades().map((item) => formatearOpcionFormulario(item, item.categoria));
...
function formatearOpcionFormulario(actividad, categoria) {
  return actividad.nombre + ' (' + categoria + ')';
}
